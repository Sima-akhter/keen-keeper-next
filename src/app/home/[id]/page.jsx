...
// // import QuickCheckIn from "@/components/clickbin/QuickCheckIn";
...
// Next.js-এ dynamic route কম্পোনেন্ট এভাবে লিখতে হয়
export default async function PageId(props) {
    // params-কে আগে অপেক্ষা (await) করে নিতে হবে
    const params = await props.params;
    const id = params.id;

    let apps = [];
    try {
        // ডাইনামিকালি কারেন্ট হোস্ট বা রিলেটিভ পাথ ব্যবহার করা নিরাপদ, 
        // তবে আপদকালীন ফিক্স হিসেবে আপনার বর্তমান সচল ডোমেনটি নিচে দেওয়া হলো:
        const res = await fetch("https://keen-keeper-next-pavr.vercel.app/data.json", {
            cache: "no-store", 
        });
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="bg-white p-4 rounded-lg text-center shadow">
                            <h2 className="text-sm font-bold">
                                {next_due_date ? new Date(next_due_date).toDateString() : "N/A"}
                            </h2>
                            <p className="text-xs text-gray-400">Next Due</p>
                        </div>
                    </div>
